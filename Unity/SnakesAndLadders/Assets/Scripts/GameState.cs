...
// This class represents states of games in the Player vs. Computer game mode.
public class GameState
{
    ...
    public List<LightweightSnake> snakeObjects = new List<LightweightSnake>();
    ...
    public GameController.Turn turnType;
    // Tracks the number of turns since last time diagonal movement was available.
    ...
    // The constructor
    public GameState(int playerPos, List<LightweightSnake> snakeObjects, int currentTurn, int maxTurns,
        int rows, int cols, List<(int, int)> ladders, GameController.Turn turnType, int turnsSinceLastDiagonal, int DiagonalEveryXTurns)
    {
    ...
    public float Utility()
    {
        ...
        {
            return turnType == GameController.Turn.Player ? float.NegativeInfinity : float.PositiveInfinity;
        }
        // Else we got here because currentTurn > maxTurns and after this if check we know the player is not in the finished square
        return turnType == GameController.Turn.Player ? float.PositiveInfinity : float.NegativeInfinity;
    }
    ...
    public float Evaluate()
    {
        ...
        if (turnType == GameController.Turn.Player)
        {
            ...
            return (getDistance(rows - 1, cols - 1, getPlayerPosition().Item1, getPlayerPosition().Item2)) * -1;
        }
        else
        {
    ...
    // Returns Euclidean distance between two points in the board
    public float getEuclideanDistance(int p1_row, int p1_col, int p2_row, int p2_col)
    {
        float Squared1 = Mathf.Pow(p1_row - p2_row, 2);
        float Squared2 = Mathf.Pow(p1_col - p2_col, 2);
        return Mathf.Sqrt(Squared1 + Squared2);
    }
    ...
    public float getPlayerMinimumDistanceToGoal()
    {
        ...
        foreach (LightweightSnake snake in snakeObjects)
        {
            if (snake != null)
            {
    ...
    private List<GameState> GenerateSuccessors(int currentSnake)
    {
        // This list will contain all the possible successors when we finish.
        List<GameState> successors = new List<GameState>();
        if (turnType == GameController.Turn.Snake) // If it's a snake's turn
        {
            ...
            foreach ((int, int) ladder in ladders)
            {
                ...
                if (startMatrixPos.Item1 - 1 == ladderStartMatrixPosition.Item1 && startMatrixPos.Item2 == ladderStartMatrixPosition.Item2)
                {
                    ...
                    if (ladderEndMatrixPosition.Item1 + 1 < rows && ladderStartMatrixPosition.Item1 + 1 < rows)
                    {
                        ...
                        GameState ngs = SnakeTurnBuildGameStateMoveLadder(snakeNewStartPosition,
            ...
            if (startMatrixPos.Item1 + 1 < rows && endMatrixPos.Item1 + 1 < rows)
            {
                ...
                GameState ngs = SnakeTurnBuildGameState(newStartPos, newEndPos);
                successors.Add(ngs);
            }
            // Snake going down
            ...
            GameState current = new GameState(playerPos, snakeObjects, currentTurn, maxTurns, rows, cols, ladders,
                GameController.Turn.Player, turnsSinceLastDiagonal, DiagonalEveryXTurns);
        ...
        else // If it's a player's turn
        {
            ...
            if (playerRow + 1 < rows)
            {
                GameState ngs = new GameState(CalculatePlayerNewPosition(playerRow + 1, playerCol), snakeObjects,
                    currentTurn + 1, maxTurns, rows, cols, ladders,
                    GameController.Turn.Snake, (turnsSinceLastDiagonal + 1) % DiagonalEveryXTurns, DiagonalEveryXTurns);
                successors.Add(ngs);
            }
            // Player going right
            if (playerCol + 1 < cols)
            {
                GameState ngs = new GameState(CalculatePlayerNewPosition(playerRow, playerCol + 1), snakeObjects,
                    currentTurn + 1, maxTurns, rows, cols, ladders,
                    GameController.Turn.Snake, (turnsSinceLastDiagonal + 1) % DiagonalEveryXTurns, DiagonalEveryXTurns);
                successors.Add(ngs);
            }
            ...
            if (CanUseDiagonal())
            {
                ...
                {
                    GameState ngs = new GameState(CalculatePlayerNewPosition(playerRow + 1, playerCol + 1), snakeObjects,
                    currentTurn + 1, maxTurns, rows, cols, ladders,
                    GameController.Turn.Snake, (turnsSinceLastDiagonal + 1) % DiagonalEveryXTurns, DiagonalEveryXTurns);
                    successors.Add(ngs);
                }
        ...
        // Return all the successors we've found
        return successors;
    }
    ...
    public int getCurrentSnake()
    {
        ...
        {
            LightweightSnake snake = snakeObjects[i];
    ...
    private bool ShouldSwitchGameState(GameState g2, GameController.Turn currentTurnType)
    {
        ...
        float distanceToGoal2 = getDistance(playerMatrixPosition2.Item1, playerMatrixPosition2.Item2,
            rows - 1, cols - 1);
        if (currentTurnType == GameController.Turn.Snake)
        {
    ...
    public (float, GameState) AlphaBetaPruning(int d, float alpha, float beta, GameController.Turn currentTurnType)
    {
        ...
        int chosenSnake = turnType == GameController.Turn.Player ? -1 : currentSnake;
        // Get the sucessors of this state
        List<GameState> succ = GenerateSuccessors(chosenSnake);
        // If this is a turn of whoever started the algorithm, we want to maximize the value.
        if (currentTurnType == turnType)
        {
            (float, GameState) maxEval = (float.NegativeInfinity, this);
            ...
            foreach (GameState child in succ)
            {
                // Recursively call AlphaBetaPruning again with the state's children
                (float, GameState) value = child.AlphaBetaPruning(d - 1, alpha, beta, currentTurnType);
        ...
        // If this is not a turn of whoever started the algorithm, we want to minimize the value.
        else
        {
            (float, GameState) minEval = (float.PositiveInfinity, this);
    ...
    private List<LightweightSnake> DeepCopySnakeList(List<LightweightSnake> originalList)
    {
        List<LightweightSnake> secondList = new List<LightweightSnake>();
        for (int i = 0; i < originalList.Count; i++)
        {
            secondList.Add(new LightweightSnake(originalList[i].startPos, originalList[i].endPos));
        }
        return secondList;
    }
    ...
    private GameState SnakeTurnBuildGameState(int newStartPos, int newEndPos)
    {
        List<LightweightSnake> snakes = DeepCopySnakeList(snakeObjects);
        ...
        {
            GameState ngs = new GameState(CalculatePlayerNewPosition(getMatrixPosition(playerPos).Item1, getMatrixPosition(playerPos).Item2, snakes), snakes, currentTurn, maxTurns, rows, cols, newLadders, GameController.Turn.Player,
                turnsSinceLastDiagonal, DiagonalEveryXTurns);
            return ngs;
        }
        GameState gs = new GameState(CalculatePlayerNewPosition(getMatrixPosition(playerPos).Item1, getMatrixPosition(playerPos).Item2, snakes), snakes, currentTurn, maxTurns, rows, cols, newLadders, GameController.Turn.Player,
                turnsSinceLastDiagonal, DiagonalEveryXTurns);
        return gs;
    }
    ...
    // This function is used as a helper function for sucessors, and it builds the next snake states when the snake
    // moves a ladder.
    private GameState SnakeTurnBuildGameStateMoveLadder(int newStartPos, int newEndPos, (int, int) ladderMoved, (int, int) ladderNewPosition)
    {
        List<LightweightSnake> snakes = DeepCopySnakeList(snakeObjects);
        snakes[currentSnake].startPos = newStartPos;
        snakes[currentSnake].endPos = newEndPos;
        ...
        if (newStartPos == playerPos)
        {
            GameState ngs = new GameState(CalculatePlayerNewPosition(getMatrixPosition(newEndPos).Item1, getMatrixPosition(newEndPos).Item2, snakes), snakes, currentTurn, maxTurns, rows, cols, newLadders, GameController.Turn.Player,
                turnsSinceLastDiagonal, DiagonalEveryXTurns);
            return ngs;
        }
        GameState gs = new GameState(CalculatePlayerNewPosition(getMatrixPosition(playerPos).Item1, getMatrixPosition(playerPos).Item2, snakes), snakes, currentTurn, maxTurns, rows, cols, newLadders, GameController.Turn.Player,
    ...
    private int CalculatePlayerNewPosition(int newPositionRow, int newPositionCol, List<LightweightSnake> snakes = null)
    {
        ...
        do
        {
            ...
            foreach (LightweightSnake snake in (snakes == null ? snakeObjects : snakes))
            {
...
public class LightweightSnake
{
    public int startPos;
    public int endPos;

    public LightweightSnake(int start, int end)
    {
        startPos = start;
        endPos = end;
    }
